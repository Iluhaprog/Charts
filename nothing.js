...
export const init = () => {
  ...
  const { canvas, context } = createCanvas({ ...SIZES, background: BACKGROUND() });
...
export const draw = (context, data) => {
  ...
  data.map(({ x, y }, index) => {
    drawPoint({ context, x, y: calcY(y), size: 2, background: LINE_COLOR(0.8) });
    if (index < testData.length - 1) {
      drawLine({
        context,
        x1: x, y1: calcY(y),
        x2: testData[index + 1].x, y2: calcY(testData[index + 1].y),
...
export const drawLine = ({ context, x1, y1, x2, y2, lineWidth, background }) => {
  context.beginPath();
